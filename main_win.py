...
def get_app_dir():
    # When frozen (Windows exe), read/write beside the executable.
...
APP_DIR = get_app_dir()
RECORD_PATH = os.path.join(APP_DIR, "record.json")
SCORE_PATH = os.path.join(APP_DIR, "score.json")


def find_reveal_image():
    # Prefer external assets next to the exe/script, then packaged fallback.
    candidates = [
        os.path.join(APP_DIR, "assets", "reveal.jpg"),
        os.path.join(APP_DIR, "assets", "reveal.jpeg"),
        os.path.join(APP_DIR, "assets", "reveal.png"),
    ]
    meipass = getattr(sys, "_MEIPASS", None)
    if meipass:
        candidates.extend(
            [
                os.path.join(meipass, "assets", "reveal.jpg"),
                os.path.join(meipass, "assets", "reveal.jpeg"),
                os.path.join(meipass, "assets", "reveal.png"),
            ]
        )
...
def config_candidates():
    paths = [os.path.join(APP_DIR, "config.json")]
    cwd_path = os.path.join(os.getcwd(), "config.json")
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.json")
    meipass = getattr(sys, "_MEIPASS", None)
    ...
    if meipass:
        meipass_path = os.path.join(meipass, "config.json")
        if meipass_path not in paths:
            paths.append(meipass_path)
    return paths
...
def load_config():
    ...
    for path in config_candidates():
        ...
        try:
            ...
            last_error = f"Config JSON error in {path}: {e}"
            continue
        except OSError as e:
            last_error = f"Config read error in {path}: {e}"
            continue
        if not isinstance(data, dict):
            last_error = f"Config format error in {path}: root must be a JSON object."
            continue
        # Accept both snake_case keys and UPPER_SNAKE_CASE keys.
        normalized = {str(k).lower(): v for k, v in data.items()}
        for k in cfg.keys():
            if k in data:
                cfg[k] = data[k]
            elif k.lower() in normalized:
                cfg[k] = normalized[k.lower()]
        loaded_from = path
        last_error = ""
        break
    return cfg, loaded_from, last_error


CONFIG, CONFIG_SOURCE, CONFIG_WARNING = load_config()
...
if CONFIG_SOURCE:
    print(f"Loaded config from: {CONFIG_SOURCE}")
else:
